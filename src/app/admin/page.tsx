...
import { differenceInDays, differenceInMonths, differenceInWeeks, eachDayOfInterval, eachMonthOfInterval, eachWeekOfInterval, eachYearOfInterval, endOfWeek, interval, max, min, startOfWeek, subDays } from "date-fns";
...
import { RANGE_OPTIONS, getRangeOptions } from "@/lib/rangeOptions";
...
async function getSalesData(createdAfter: Date | null, createdBefore: Date | null) {
    ...
    const {array,format} = getChartDateArray(createdAfter || chartData[0].createdAt, createdBefore || new Date())
    
    const dayArray=array.map(date => {
        return {
            date: format(date), totalSales: 0
        }
    })
...
async function getProductData(createdAfter: Date | null, createdBefore: Date | null) {
    ...
    const [activeCount, inactiveCount, chartData] = await
        Promise.all([
            db.product.count({ where: { isAvailableForPurchase: true } }),

            db.product.count({ where: { isAvailableForPurchase: false } }),

            db.product.findMany({ select: { name: true, orders: { select: { pricePaidInCents: true }, where: { createdAt: createdAtQuery } } } })
        ])



    return {
        activeCount, inactiveCount, chartData: chartData.map(product => {
            return {
                name: product.name,
...
async function getUserData(createdAfter: Date | null, createdBefore: Date | null) {
    ...
    const [userCount, orderData, chartData] = await Promise.all([db.user.count(), db.order.aggregate({
        _sum: { pricePaidInCents: true }
    }),
    db.user.findMany({ select: { createdAt: true }, where: { createdAt: createdAtQuery }, orderBy: { createdAt: "asc" } })
    ])
    const {array,format} =getChartDateArray(createdAfter || chartData[0].createdAt, createdBefore || new Date())
    
    const dayArray=array.map(date => {
        return {
            date: format(date), totalUsers: 0
        }
    })
    ...
    return {
        chartData: chartData.reduce((data, user) => {
            ...
            entry.totalUsers += 1
            return data
        }, dayArray), userCount, averageValuePerUser: userCount === 0 ? 0 : (orderData._sum.pricePaidInCents || 0) / userCount / 100
    }
}

export default async function AdminDashboard({ searchParams: { totalSalesRange,totalSalesRangeFrom,totalSalesRangeTo,newCustomersRange,newCustomersRangeFrom,newCustomersRangeTo, revenueByProductRange,revenueByProductRangeFrom,revenueByProductRangeTo } }: {
    searchParams: {
    ...
    const totalSalesRangeOption = getRangeOptions(totalSalesRange,totalSalesRangeFrom,totalSalesRangeTo) || RANGE_OPTIONS.last_7_days
    const newCustomersRangeOption = getRangeOptions(newCustomersRange,newCustomersRangeFrom,newCustomersRangeTo) || RANGE_OPTIONS.last_7_days
    const revenueByProductRangeOption = getRangeOptions(revenueByProductRange,revenueByProductRangeFrom,revenueByProductRangeTo) || RANGE_OPTIONS.last_7_days
    ...
    return <>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <DashboardCard title="Customers" subtitle={`${formatNumber(userData.averageValuePerUser)} Average Value`} body={formatNumber(userData.userCount)} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-8">
            <ChartCard title="Total Sales" queryKey="totalSalesRange" selectedRangeLabel={totalSalesRangeOption.label}><OrdersByDayChart data={salesData.chartData} /></ChartCard>
            <ChartCard title="New Customers" queryKey="newCustomersRange" selectedRangeLabel={newCustomersRangeOption.label}><UsersByDayChart data={userData.chartData} /></ChartCard>
            <ChartCard title="Revenue By Product" queryKey="revenueByProductRange" selectedRangeLabel={revenueByProductRangeOption.label}><RevenueByProductChart data={productData.chartData} /></ChartCard>
        </div>
    </>
}
