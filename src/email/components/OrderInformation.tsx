import { formatCurrency } from "@/lib/formatter"
import { Button, Column, Img, Row, Section,Text } from "@react-email/components"

type OrderInfromationProps={
    order:{id:string,createdAt:Date,pricePaidInCents:number},
    product:{name:string,imagePath:string,description:string},
    downloadVerificationId:string
}


const dateFormatter=new Intl.DateTimeFormat("en",{dateStyle:"medium"})



const OrderInformation = ({order,product,downloadVerificationId}:OrderInfromationProps) => {
  return (
    <>
    <Section>
        <Row>
            <Column>
            <Text className="mb-0 text-gray-500 whitespace-nowrap text-nowrap">Order ID</Text>
            <Text>{order.id}</Text>
            </Column>
            <Column>
            <Text className="mb-0 text-gray-500 whitespace-nowrap text-nowrap">Purchased On</Text>
            <Text>{dateFormatter.format(order.createdAt)}</Text>
            </Column>
            <Column>
            <Text className="mb-0 text-gray-500 whitespace-nowrap text-nowrap">Price Paid</Text>
            <Text>{formatCurrency(order.pricePaidInCents/100)}</Text>
            </Column>
        </Row>
    </Section>
    <Section className="border border-solid border-gray-500 rounded-lg p-4 md:p-6 my-4">
        <Img src={`${process.env.NEXT_PUBLIC_SERVER_URL}${product.imagePath}`} alt={product.name} width="100%"/>
        <Row className="mt-8">
          <Column className="align-bottom">
            <Text className="text-lg font-bold m-0 mr-4">{product.name}</Text>
          </Column>
          <Column className="right">
          <Button href={`${process.env.NEXT_PUBLIC_SERVER_URL}/products/download/${downloadVerificationId}`} className="bg-black text-white px-6 py-4">
          </Button>
          </Column>
        </Row>
        <Row>
          <Column>
          <Text className="text-gray-500 mb-0">{product.description}</Text>
          </Column>
        </Row>
    </Section>
    </>
  )
}

export default OrderInformation